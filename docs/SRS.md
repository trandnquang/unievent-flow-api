...
| Trạng thái | Bổ sung — dùng để định hình & thống nhất phạm vi dự án |
...
[1\. Giới thiệu	3](#1.-giới-thiệu)

[1.1 Mục đích tài liệu	3](#1.1-mục-đích-tài-liệu)

[1.2 Phạm vi dự án	3](#1.2-phạm-vi-dự-án)

[1.3 Đối tượng sử dụng tài liệu	3](#1.3-đối-tượng-sử-dụng-tài-liệu)

[1.4 Định nghĩa & từ viết tắt	3](#1.4-định-nghĩa-&-từ-viết-tắt)

[2\. Mô tả tổng quan	5](#2.-mô-tả-tổng-quan)

[2.1 Bối cảnh & vấn đề cần giải quyết	5](#2.1-bối-cảnh-&-vấn-đề-cần-giải-quyết)

[2.2 Tầm nhìn sản phẩm	5](#2.2-tầm-nhìn-sản-phẩm)

[2.3 Đối tượng người dùng (Actors)	5](#2.3-đối-tượng-người-dùng-\(actors\))

[2.4 Giả định & ràng buộc	5](#2.4-giả-định-&-ràng-buộc)

[2.5 Phân chia vai trò trong nhóm (2 người)	5](#2.5-phân-chia-vai-trò-trong-nhóm-\(2-người\))

[3\. Yêu cầu chức năng (Functional Requirements)	6](#3.-yêu-cầu-chức-năng-\(functional-requirements\))

[4\. Yêu cầu phi chức năng (Non-Functional Requirements)	7](#4.-yêu-cầu-phi-chức-năng-\(non-functional-requirements\))

[5\. Kiến trúc hệ thống đề xuất	8](#5.-kiến-trúc-hệ-thống-đề-xuất)

[5.1 Kiến trúc tổng quan	8](#5.1-kiến-trúc-tổng-quan)

[5.2 Luồng đăng ký vé chống bán vượt (oversell)	8](#5.2-luồng-đăng-ký-vé-chống-bán-vượt-\(oversell\))

[5.3 Luồng check-in bằng mã QR (JWT)	8](#5.3-luồng-check-in-bằng-mã-qr-\(jwt\))

[5.4 Luồng phân tích cảm xúc phản hồi (AI)	8](#5.4-luồng-phân-tích-cảm-xúc-phản-hồi-\(ai\))

[5.5 Điều chỉnh quan trọng để đảm bảo khả thi (đã rà soát so với mô tả gốc)	8](#5.5-điều-chỉnh-quan-trọng-để-đảm-bảo-khả-thi-\(đã-rà-soát-so-với-mô-tả-gốc\))

[6\. Yêu cầu dữ liệu	9](#6.-yêu-cầu-dữ-liệu)

[6.1 Các thực thể chính	9](#6.1-các-thực-thể-chính)

[6.2 Quan hệ giữa các thực thể (tóm tắt)	9](#6.2-quan-hệ-giữa-các-thực-thể-\(tóm-tắt\))

[7\. Yêu cầu giao diện ngoài (External Interfaces)	10](#7.-yêu-cầu-giao-diện-ngoài-\(external-interfaces\))

[7.1 Giao diện người dùng	10](#7.1-giao-diện-người-dùng)

[7.2 Giao diện API	10](#7.2-giao-diện-api)

[7.3 Giao diện phần cứng	10](#7.3-giao-diện-phần-cứng)

[7.4 Dịch vụ/phần mềm bên thứ ba	10](#7.4-dịch-vụ/phần-mềm-bên-thứ-ba)

[8\. Ma trận Use Case tóm tắt	11](#8.-ma-trận-use-case-tóm-tắt)

[9\. Kế hoạch triển khai 7 tuần	12](#9.-kế-hoạch-triển-khai-7-tuần)

[10\. Rủi ro & Giải pháp giảm thiểu	13](#10.-rủi-ro-&-giải-pháp-giảm-thiểu)

[11\. Tiêu chí nghiệm thu cho bản demo (Definition of Done)	14](#11.-tiêu-chí-nghiệm-thu-cho-bản-demo-\(definition-of-done\))

# **1\. Giới thiệu** {#1.-giới-thiệu}

## **1.1 Mục đích tài liệu** {#1.1-mục-đích-tài-liệu}
...
## **1.2 Phạm vi dự án** {#1.2-phạm-vi-dự-án}
...
## **1.3 Đối tượng sử dụng tài liệu** {#1.3-đối-tượng-sử-dụng-tài-liệu}
...
## **1.4 Định nghĩa & từ viết tắt** {#1.4-định-nghĩa-&-từ-viết-tắt}
...
## **2.1 Bối cảnh & vấn đề cần giải quyết** {#2.1-bối-cảnh-&-vấn-đề-cần-giải-quyết}

Các câu lạc bộ, tổ chức và phòng ban sinh viên hiện tổ chức rất nhiều sự kiện mỗi năm, nhưng công tác truyền thông và đăng ký thường rời rạc qua mạng xã hội và Google Forms. Cách làm này tạo ra hai lỗ hổng chính: (1) không kiểm soát được sức chứa sự kiện theo thời gian thực, dẫn tới nhận đăng ký vượt số lượng chỗ; và (2) không có công cụ theo dõi người tham dự thực tế (check-in) lẫn phân tích phản hồi sau sự kiện một cách có hệ thống.

## **2.2 Tầm nhìn sản phẩm** {#2.2-tầm-nhìn-sản-phẩm}

UniEvent Flow là nền tảng quản lý sự kiện học đường theo mô hình từ gốc đến ngọn: từ tạo sự kiện, đăng ký, phát vé điện tử, check-in tại cổng, cho đến thu thập và phân tích phản hồi sau sự kiện. Giá trị cốt lõi của dự án không nằm ở các form CRUD thông thường, mà ở hai bài toán kỹ thuật thực tế: (1) xử lý đăng ký đồng thời với số lượng vé giới hạn mà không bị bán vượt, và (2) xác thực vé tại cổng với độ trễ cực thấp để tránh ùn tắc.

## **2.3 Đối tượng người dùng (Actors)** {#2.3-đối-tượng-người-dùng-(actors)}
...
## **2.4 Giả định & ràng buộc** {#2.4-giả-định-&-ràng-buộc}
...
## **2.5 Phân chia vai trò trong nhóm (2 người)** {#2.5-phân-chia-vai-trò-trong-nhóm-(2-người)}

| Thành viên | Phạm vi phụ trách |
| :---- | :---- |
| Trần Đình Nhật Quang— Backend & Kiến trúc | Thiết kế cơ sở dữ liệu, xây dựng luồng Redis chống bán vượt vé, tích hợp hàng đợi (BullMQ), API check-in tốc độ cao, xác thực JWT, cấu hình server/domain & triển khai (deploy) |
| Hồ Tiến Dũng — Frontend & Tích hợp | Xây dựng giao diện ReactJS (trang sự kiện, form đăng ký), chức năng quét mã QR qua camera trình duyệt, dashboard biểu đồ (Recharts) hiển thị dữ liệu phân tích cảm xúc |
...
Mỗi yêu cầu được gắn mức độ ưu tiên: MVP (bắt buộc, không có không demo được), Should (nên có nếu đủ thời gian), Could (có thì tốt, cắt đầu tiên nếu trễ tiến độ).
...
| FR-07 | Quên mật khẩu (gửi link đặt lại qua email)   | Sinh viên & Ban tổ chức | Could |

## **3.2 Quản lý sự kiện (Event Management)**

| ID | Mô tả chức năng | Actor | Ưu tiên |
| ----- | :---- | :---- | ----- |
| FR-08 | Tạo sự kiện mới kèm landing page (ảnh bìa, mô tả, thời gian, địa điểm, số vé tối đa)   | Ban tổ chức   | MVP |
| FR-09 | Xem trang chi tiết sự kiện (landing page): mô tả, thời gian, địa điểm, số vé còn lại theo thời gian thực  | Sinh viên | MVP |
| FR-10 | Sửa thông tin sự kiện đã tạo   | Ban tổ chức  | Should |
| FR-11 | Xoá / Huỷ sự kiện — chuyển trạng thái "đã huỷ" thay vì xoá cứng nếu đã phát hành vé    | Ban tổ chức  | Could  |
| FR-12 | Xem danh sách sự kiện hiện tại đang phụ trách   | Ban tổ chức  | MVP |
| FR-13 | Tìm kiếm, lọc sự kiện theo CLB/phòng ban, loại hình, khoảng thời gian   | Sinh viên | MVP |

## **3.3 Đăng ký & Vé điện tử (Registration & Ticket)**

| ID | Mô tả chức năng | Actor | Ưu tiên |
| ----- | :---- | :---- | ----- |
| FR-14 | Đăng ký / đặt vé tham dự sự kiện   | Sinh viên    | MVP |
| FR-15 | Sinh mã vé QR chứa chuỗi JWT mã hoá thông tin vé   | Hệ thống  | MVP |
| FR-16 | Gửi vé điện tử qua email bất đồng bộ qua hàng đợi, không chặn luồng chính của server    | Hệ thống   | MVP |
| FR-17 | Xem danh sách vé / lịch sử đăng ký sự kiện của cá nhân     | Sinh viên   | Should  |
| FR-18 | Xem chi tiết một vé cụ thể    | Sinh viên   | Should  |

## **3.4 Check-in tại cổng sự kiện (Gate Check-in)**

| ID | Mô tả chức năng | Actor | Ưu tiên |
| ----- | :---- | :---- | ----- |
| FR-19 | Xác thực và giải mã mã QR tại thời điểm check-in, độ trễ dưới 1 giây    | Hệ thống     | MVP |
| FR-20 | Ghi nhận check-in (đổi trạng thái vé, ghi CheckinLog)    | Hệ thống  | MVP |
| FR-21 | Xem lịch sử check-in tại cổng sự kiện     | Ban tổ chức    | Should  |
| FR-22 | Xuất danh sách người tham dự đã check-in ra file CSV      | Ban tổ chức    | Could  |

## **3.5 Phản hồi & Phân tích cảm xúc AI (Feedback & AI Sentiment)**

| ID | Mô tả chức năng | Actor | Ưu tiên |
| ----- | :---- | :---- | ----- |
| FR-23 | Gửi đánh giá / phản hồi (feedback) sau khi đã tham dự sự kiện    | Sinh viên      | MVP |
| FR-24 | Xem danh sách phản hồi đã nhận được cho sự kiện     | Ban tổ chức   | Should  |
| FR-25 | Gọi LLM API phân tích cảm xúc theo phương pháp Prompt Engineering, gộp feedback thành batch mỗi lần gọi      | Hệ thống     | MVP |
| FR-26 | Lưu & gắn nhãn cảm xúc (Tích cực/Tiêu cực/Trung lập) cùng từ khoá phàn nàn vào Feedback       | Hệ thống     | MVP |

## **3.6 Dashboard & Báo cáo thống kê (Dashboard & Statistics)**

| ID | Mô tả chức năng | Actor | Ưu tiên |
| ----- | :---- | :---- | ----- |
| FR-27 | Xem dashboard thống kê số lượng / tỷ lệ người đăng ký theo thời gian thực     | Ban tổ chức       | Should  |
| FR-28 | Xem báo cáo phân loại cảm xúc và từ khoá phàn nàn phổ biến từ feedback      | Ban tổ chức    | MVP  |
...
Đây là nhóm yêu cầu tạo nên giá trị kỹ thuật thật sự của đề tài, và là phần hội đồng có xu hướng hỏi sâu nhất khi bảo vệ.
...
| NFR\-07 | Khả năng triển khai: hệ thống deploy được trên hạ tầng free-tier | Deploy bản demo thành công trước Tuần 6 |
...
## **5.1 Kiến trúc tổng quan** {#5.1-kiến-trúc-tổng-quan}
...
## **5.2 Luồng đăng ký vé chống bán vượt (oversell)** {#5.2-luồng-đăng-ký-vé-chống-bán-vượt-(oversell)}

* Client gửi yêu cầu đăng ký vé cho một sự kiện.

* Backend thực hiện lệnh giảm số đếm nguyên tử (atomic decrement) trên Redis cho sự kiện đó.

* Nếu còn vé: tạo một bản ghi Registration ở trạng thái giữ chỗ tạm (TTL vài phút), đẩy job vào hàng đợi.

* Worker xử lý hàng đợi ghi dữ liệu chính thức vào PostgreSQL, sinh vé JWT, gửi email — toàn bộ diễn ra bất đồng bộ, không chặn request gốc.

* Nếu hết vé: Redis trả về ngay lập tức, backend phản hồi lỗi "hết vé" mà không cần chạm tới cơ sở dữ liệu chính.

## **5.3 Luồng check-in bằng mã QR (JWT)** {#5.3-luồng-check-in-bằng-mã-qr-(jwt)}

* Vé được sinh dưới dạng mã QR chứa một chuỗi JWT đã ký, mã hóa ticket\_id, event\_id và thời điểm phát hành.
...
## **5.4 Luồng phân tích cảm xúc phản hồi (AI)** {#5.4-luồng-phân-tích-cảm-xúc-phản-hồi-(ai)}
...
## **6.1 Các thực thể chính** {#6.1-các-thực-thể-chính}

| Thực thể | Trường chính | Mô tả |
| :---- | :---- | :---- |
| User | id, name, email, password\_hash, role (student/organizer), is\_active, created\_at, updated\_at, reset\_token, reset\_token\_expires  | Tài khoản dùng chung cho sinh viên và ban tổ chức, phân biệt bằng role |
| Event | id, organizer\_id,  title, description, cover\_image, location, category, club\_name, start\_time, end\_time, max\_tickets, status (active/cancelled), created\_at, updated\_at | Thông tin sự kiện, hiển thị trên landing page |
| Registration | id, event\_id, user\_id, status (pending/confirmed/failed), requested\_at, processed\_at | Yêu cầu giữ vé tạm thời, được xử lý bất đồng bộ qua hàng đợi |
| Ticket | id, registration\_id, jwt\_code, status (valid/checked\_in/cancelled), issued\_at | Vé chính thức đã phát hành, chứa mã QR/JWT |
| Feedback | Id, ticket\_id,  event\_id, user\_id, content, sentiment\_label, keywords, analyzed\_at  created\_at | Phản hồi của sinh viên sau sự kiện, có gắn nhãn cảm xúc từ AI |
| CheckinLog | id, ticket\_id, checkin\_time, organizer\_id  | Lịch sử quét vé tại cổng sự kiện |

## **6.2 Quan hệ giữa các thực thể (tóm tắt)** {#6.2-quan-hệ-giữa-các-thực-thể-(tóm-tắt)}
...
## **7.1 Giao diện người dùng** {#7.1-giao-diện-người-dùng}
...
## **7.2 Giao diện API** {#7.2-giao-diện-api}

* REST API, dữ liệu trao đổi dạng JSON, xác thực bằng JWT Bearer token trong header Authorization.

## **7.3 Giao diện phần cứng** {#7.3-giao-diện-phần-cứng}

* Không cần thiết bị quét chuyên dụng — dùng camera điện thoại/laptop qua trình duyệt (WebRTC getUserMedia) để quét mã QR.

## **7.4 Dịch vụ/phần mềm bên thứ ba** {#7.4-dịch-vụ/phần-mềm-bên-thứ-ba}

| Dịch vụ | Vai trò |
| :---- | :---- |
| Email service (ví dụ Resend/SendGrid free-tier) | Gửi vé điện tử và thông báo cho sinh viên |
| LLM API (Gemini/OpenAI) | Phân tích cảm xúc & trích xuất từ khoá phàn nàn từ feedback |
| Redis (self-host hoặc Upstash free-tier) | Đếm số vé còn lại theo thời gian thực \+ nền tảng cho hàng đợi BullMQ |
...
| :---- | :---- |
| Sinh viên | Tìm & xem sự kiện · Đăng ký vé · Nhận vé QR · Xem lịch sử vé · Gửi feedback |
...
| ----- | :---- | ----- | :---- |
| 1 | Redis \+ hàng đợi (BullMQ) là công nghệ mới với cả nhóm | Cao | Làm PoC nhỏ (một luồng giữ vé hoàn chỉnh) trong Tuần 3, trước khi build tính năng đầy đủ |
