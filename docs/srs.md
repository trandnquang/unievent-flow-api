...
_**Phiên bản: v0.6.8** — FR-01 → FR-42 (42 FR), 42 UC, 127 BR. Đồng bộ với API v0.4.6, ERD v0.4.1, SCHEMA v0.4.1._
...
flowchart TD
    ...
    U -->|Roi| V[422 EVENT_ALREADY_CANCELLED - MSG-34]
    U -->|Chua| W[(status=cancelled, ghi cancelled_by/at BR-106)]
    W --> X[(Toan bo ticket valid chuyen cancelled)]
    X --> Y[[Huy job nhac lich BR-97]]

    J -->|Moi Co-host| Z{Nguoi duoc moi co role=organizer? BR-46}
    Z -->|Khong| Z1[422 CO_HOST_NOT_ORGANIZER - MSG-31]
    Z -->|Co| Z2[(Tao event_co_hosts status=pending BR-45)]
    Z2 --> Z3{Nguoi duoc moi phan hoi}
    Z3 -->|Chap nhan| Z4[(status=accepted BR-46b)]
    Z3 -->|Tu choi| Z5[(status=declined)]
...
Kiểm tra điều kiện tiên quyết (BR-87: đúng vai trò Sinh viên, sự kiện đang active, chưa tới giờ bắt đầu) → Redis atomic decrement (còn vé/hết vé) → đặt khoá giữ chỗ TTL 60 giây (BR-88) → BullMQ sinh vé + gửi email → nếu worker thất bại hoặc hết TTL: hoàn 1 vé về Redis, Registration chuyển `failed` (BR-89) → Sinh viên xem vé → có thể tự huỷ (Registration **và** Ticket cùng chuyển `cancelled`, hoàn vé về Redis — BR-55/56) → hệ thống tự gửi email nhắc lịch trước giờ diễn ra cho các đăng ký còn hiệu lực tại thời điểm gửi (BR-58).
...
flowchart TD
    ...
    D -->|Co| E[409 DUPLICATE_REGISTRATION]
    ...
    I --> J[/Dat khoa giu cho hold:registrationId TTL 60s BR-88/]
    J --> K[[Day job sinh ve vao BullMQ]]
    K --> L[Tra 202 Accepted kem registrationId]

    L --> M{Worker xu ly}
    M -->|Thanh cong| N[Sinh JWT ve, exp = end_time + 24h BR-99, BR-51]
    N --> O[(tickets status=valid; registrations status=confirmed)]
    O --> P[/Xoa khoa hold BR-51/]
    P --> Q[[Gui email xac nhan kem QR]]

    M -->|That bai HOAC het TTL 60s| R{BR-93: UPDATE ... WHERE status='pending' anh huong 1 dong?}
...
flowchart TD
    ...
    Q -->|Roi| R[422 EVENT_ALREADY_CANCELLED - MSG-34]
...
- `pending → failed`: worker xử lý thất bại **hoặc** hết TTL giữ chỗ 60 giây (BR-88, BR-89). Cả hai nhánh đều **hoàn 1 vé về bộ đếm Redis**.
...
flowchart TB
    ...
    subgraph EXT[Dich vu ben ngoai]
        ...
        LLMAPI[LLM API<br/>Phan tich cam xuc]
        STORAGE[Cloudinary / Supabase Storage<br/>Luu tru anh - FR-40]
    end
...
1. **Redis đảm nhiệm năm vai trò khác nhau**, không chỉ là bộ nhớ đệm: bộ đếm tồn kho vé (BR-47, BR-89, BR-90), khoá nguyên tử chống check-in trùng (BR-91) và khoá giữ chỗ (BR-88), hàng đợi công việc của BullMQ, giới hạn tần suất đăng nhập (NFR 6.1), và cache trạng thái tài khoản (CBR 7). Việc dồn nhiều vai trò vào một thành phần là đánh đổi có ý thức — giảm số hạ tầng phải vận hành trong 7 tuần, đổi lấy việc Redis trở thành điểm hỏng đơn (single point of failure); hệ quả và cách giảm thiểu được nêu ở NFR 6.6.
...
| **Post-condition:** | Thông tin sự kiện được hiển thị chính xác, gồm ticketsRemaining thời gian thực.     |
...
| (4)      | **BR-33**              | Real-time Ticket Count Rule: Trường ticketsRemaining đọc trực tiếp từ Redis tại thời điểm request, không đọc từ PostgreSQL, đảm bảo phản ánh đúng số vé thực tế còn lại.                                                                                                                                                                                                                                                                                                             |
| (4b)     | **BR-33b** ⭐ mới v1.0 | Public Registered Count Rule: `GET /events/:id` (FR-09) và mỗi item của `GET /events` (FR-13) trả kèm `registeredCount` = số đăng ký đang chiếm chỗ (`registrations.status IN ('confirmed','pending')`), dùng hiển thị công khai “X người tham gia”. Đây chỉ là **con số tổng hợp**, không lộ danh tính người đăng ký — danh sách chi tiết (kèm PII) thuộc FR-41 và chỉ dành cho người vận hành. Khi `ticketsRemaining = 0`, giao diện hiển thị trạng thái hết vé (SOLD_OUT/MSG-23). |
...
| (5)      | **BR-37**   | Soft-cancel Rule: Đổi status → cancelled, không dùng DELETE và không xoá dữ liệu (soft-cancel). Các tickets liên quan chuyển sang cancelled.                                                  |
...
| (7)      | **BR-37c**  | Idempotency Rule: Nếu event.status đã là cancelled, từ chối huỷ lại, trả lỗi EVENT_ALREADY_CANCELLED (HTTP 409).                                                                              |
...
| (6)      | **BR-40b** ⭐ v0.6.4 | Edit Rule: `PATCH /events/:eventId/updates/:updateId` dùng `requireOwnerOrCoHost` (như POST). `updateId` phải thuộc đúng `eventId` (khác → 404). Sửa **chỉ thay đổi bản ghi trong feed**, **không** gửi lại email — email đã phát ở lần đăng đầu không thu hồi/không đồng bộ được (giới hạn có chủ đích, nêu rõ trên UI). |
...
| (10)     | **BR-46d**  | Self-Confirmation Rule: PATCH /events/:eventId/co-hosts/me/accept và PATCH /events/:eventId/co-hosts/me/decline chỉ tác động lên bản ghi có user_id = req.user.id (lấy từ JWT, theo CBR 3) — không nhận userId từ path/body. Không tồn tại bản ghi pending tương ứng → HTTP 404.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Post-condition:** | Registration được tạo ở trạng thái pending kèm khoá giữ chỗ TTL 60 giây, đang chờ worker xử lý thành confirmed hoặc failed.         |
...
| (7)      | **BR-88**   | Hold TTL Rule: Đồng thời với việc tạo Registration, hệ thống đặt một khoá giữ chỗ trên Redis theo mẫu \`hold:{registrationId}\` với TTL = 60 giây (giá trị cấu hình được qua biến môi trường; 60 giây được chọn vì gấp nhiều lần thời gian xử lý bình thường của worker nhưng vẫn đủ ngắn để không giam vé lâu khi hệ thống trục trặc). Khoá hết hạn mà Registration vẫn còn ở trạng thái pending ⇒ coi như xử lý thất bại: áp dụng BR-89. Lý do cần quy tắc này: nếu chỉ nói tới khái niệm “TTL giữ chỗ” mà không định nghĩa giá trị, nơi lưu và bên chịu trách nhiệm quét hết hạn, thì một job biến mất khỏi hàng đợi (worker chết giữa chừng, Redis khởi động lại) sẽ khiến vé bị giam vĩnh viễn mà không cơ chế nào phát hiện được. |
...
flowchart TB
    ...
    direction TB
        ...
        N5["(5) Registration = failed (worker lỗi / hết TTL)"]
...
| (6)      | **BR-89**   | Ticket Compensation Rule: Nếu worker xử lý thất bại (lỗi CSDL, hết số lần retry của BullMQ) hoặc khoá giữ chỗ hết hạn khi Registration vẫn ở pending (BR-88), hệ thống thực hiện đúng 2 việc trong cùng một khối xử lý lỗi: (a) đặt registrations.status = failed, processed_at = now(); (b) INCR lại 1 đơn vị bộ đếm vé còn lại trên Redis, đối xứng với thao tác giảm ở BR-47. Ghi log mức WARN kèm registrationId để đối soát. Lý do bắt buộc phải có: vé đã bị trừ trên Redis ở BR-47; nếu không có đường hoàn lại khi luồng thất bại thì mỗi job lỗi làm “bốc hơi” vĩnh viễn 1 vé, khiến sự kiện 100 vé báo hết vé ở người thứ 95. Hệ thống khi đó chống được oversell nhưng lại sinh ra undersell; cả hai đều là lỗi kiểm soát tồn kho và đều mâu thuẫn với mục tiêu nghiệp vụ nêu ở mục 1.2. |
| (5)      | **BR-93**   | Idempotent Compensation Rule: Thao tác hoàn vé ở BR-89 chỉ được thực hiện đúng một lần cho mỗi Registration. Cơ chế: chỉ hoàn vé khi câu lệnh \`UPDATE registrations SET status='failed' WHERE id=? AND status='pending'\` trả về đúng 1 dòng bị ảnh hưởng — nếu trả về 0 dòng nghĩa là bản ghi đã được xử lý bởi một luồng khác (worker retry và bộ quét TTL cùng chạm vào một bản ghi), khi đó không hoàn vé lần nữa. Nếu thiếu quy tắc này, chính cơ chế bù trừ lại trở thành nguồn gây oversell.                                                                                                                                                                                                                                                                                                |
...
| **Step** | **BR Code** | **Description**                                                                                                                                                                             |
| -------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (2)      | **BR-72**   | Batch Rule: Gộp các feedback có content khác rỗng và chưa phân tích (analyzed_at IS NULL) thành 1 batch, gọi LLM API. Feedback chỉ có rating, không có content, có thể bỏ qua bước gọi LLM. |
| (3)      | **BR-73**   | Trigger Rule: Có thể kích hoạt thủ công (nút “Phân tích ngay” trên dashboard) hoặc tự động theo lịch (cron mỗi N giờ).                                                                      |
...
| **Step** | **BR Code** | **Description**                                                                                                                                                               |
| -------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (2)      | **BR-74**   | Persistence Rule: Lưu sentiment_label (positive/negative/neutral), keywords, analyzed_at = now() sau khi LLM trả kết quả. Feedback chưa phân tích giữ sentiment_label = NULL. |
...
| (6)      | **BR-98**   | Immediate Revocation Rule: Việc vô hiệu hoá có hiệu lực từ request kế tiếp, không chờ accessToken hiện tại hết hạn. Thực thi bằng middleware \`requireActive\` (CBR 7). Khi FR-29 đổi trạng thái, hệ thống xoá ngay khoá cache \`active:{userId}\` trên Redis để không phải chờ hết TTL. Vấn đề được xử lý: nếu \`is_active\` chỉ được kiểm tra tại bước đăng nhập (BR-08), một tài khoản vừa bị vô hiệu hoá vẫn thao tác bình thường tối đa 2 giờ — đúng bằng TTL của accessToken. Với một chức năng mà mục đích chính là xử lý người dùng vi phạm chính sách, độ trễ 2 giờ là lỗ hổng thật chứ không phải chi tiết học thuật.                                                                                                                      |
...
| (5)      | **BR-102**  | Self-Protection Rule: Danh sách trả về có gắn cờ để giao diện vô hiệu hoá nút thao tác trên chính tài khoản Quản trị viên đang đăng nhập; ở tầng backend, FR-29 từ chối request có userId trùng với req.user.id (HTTP 422). Ngăn tình huống Quản trị viên tự khoá tài khoản của mình và làm hệ thống mất hoàn toàn quyền quản trị — vì không có luồng nào tạo lại tài khoản admin ngoài script seed (Assumption #11). |
...
| (6)      | **BR-111**  | Storage & URL Persistence Rule: Tệp được đẩy lên dịch vụ lưu trữ bên thứ ba (Cloudinary hoặc Supabase Storage — xem Assumption #13); hệ thống chỉ lưu URL trả về vào CSDL, không lưu tệp nhị phân trên máy chủ ứng dụng và không lưu vào PostgreSQL. Dịch vụ lưu trữ không phản hồi hoặc trả lỗi → HTTP 502 UPLOAD_FAILED (MSG-48), không tạo bản ghi nào. Endpoint chỉ trả về URL; việc gán URL đó vào sự kiện hay hồ sơ là một request riêng (FR-08/FR-31/FR-06) — tách hai bước giúp người dùng xem trước ảnh trước khi lưu, và tránh việc một lần tải ảnh thất bại làm hỏng cả thao tác tạo sự kiện. |
...
3. **Hàng chip lọc nhanh** — Tất cả, Hôm nay, Tuần này, và các **danh mục ENUM** (Workshop, Âm nhạc, Thể thao, CLB…). ⭐ **v1.0:** _không_ có chip “Sắp hết vé” — hệ thống **không** xử lý logic lọc/sắp theo mức sắp hết vé (T4 đã loại); thẻ sự kiện chỉ thể hiện 2 trạng thái **đăng ký được** / **hết vé** (`ticketsRemaining = 0`). “Hôm nay/Tuần này” chỉ là preset của `from/to`.
4. **“Sắp diễn ra”** — 3 thẻ nổi bật (sự kiện sắp diễn ra sớm nhất theo `start_time`, **suy ra**, không có cột `is_featured`). Mỗi thẻ: ảnh bìa, badge đếm ngược, danh mục, tiêu đề, thanh tiến độ vé + `ticketsRemaining`, nút Đăng ký nhanh.
5. **“Tất cả sự kiện”** — danh sách thẻ (ảnh, ngày, địa điểm, đơn vị tổ chức, **“X người tham gia”** = `registeredCount`, badge trạng thái) + phân trang.
...
⭐ **v1.0 — thành phần & trạng thái bắt buộc.** Trang phải hiển thị đầy đủ dữ liệu mà `GET /events/:id` (FR-09) đã trả: **Giới thiệu**, **Lịch trình đầy đủ** (mảng `schedule`, không chỉ giờ bắt đầu/kết thúc), **Thông báo** (`updates`, 5 mới nhất), **Ban tổ chức + Co-host** (`coHosts` đã accepted), **Địa điểm** (bản đồ với `in_person`, hoặc nút vào phòng với `online`), và **hộp vé** hiển thị `ticketsRemaining` + `registeredCount` (“X người tham gia”).

**Các trạng thái đăng ký phải vẽ (state matrix):**

| Trạng thái            | Điều kiện                                                   | Giao diện                                                                                                   |
| --------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Đang mở đăng ký       | `status=active`, `ticketsRemaining > 0`, `start_time > now` | Nút **Đăng ký** hoạt động, hiện số vé còn lại                                                               |
| **Hết vé (Sold-out)** | `ticketsRemaining = 0`                                      | Nút **khoá** “Đã hết vé”, thông báo _“Sự kiện đã đủ số lượng đăng ký tối đa”_ (MSG-23), thanh vé đầy màu đỏ |
...
| **FR-11**             | Huỷ sự kiện                                  | UC-12        | BR-36, BR-37, BR-37b, BR-37c                                | —                      | `POST /events/:id/cancel`                                | 4.3.1 / 4.3.3 (thao tác) | TC-11-01→nn   |
...
| **FR-14**             | Đăng ký / đặt vé                             | UC-18        | BR-87, BR-47, BR-48, BR-49, BR-50, BR-88                    | MSG-42                 | `POST /events/:id/registrations`                         | 4.4.1                    | TC-14-01→nn   |
| **FR-15**             | Sinh mã vé QR/JWT                            | UC-19        | BR-51, BR-99, BR-109, BR-89, BR-93                          | MSG-45                 | `GET /registrations/:id`                                 | 4.4.2                    | TC-15-01→nn   |
...
| **FR-30**             | Buộc huỷ sự kiện                             | UC-37        | BR-81, BR-96, BR-106                                        | MSG-34                 | `POST /admin/events/:eventId/force-cancel`               | 4.8.2                    | TC-30-01→nn   |
...
| **FR-34**             | Tự huỷ đăng ký                               | UC-23        | BR-55, BR-56                                                | MSG-25, MSG-32         | `POST /registrations/:id/cancel`                         | 4.4.4                    | TC-34-01→nn   |
...
| **FR-37**             | Gắn Co-host, Chấp nhận/Từ chối lời mời       | UC-17        | BR-44, BR-45, BR-45b, BR-46, BR-46b, BR-46c, BR-46d, BR-46e | —                      | `PATCH /events/:eventId/co-hosts/me/accept + /decline`   | 4.3.6                    | TC-37-01→nn   |
...
| AC-14-04 | **Given** cùng một Registration `pending` bị cả worker retry và bộ quét TTL xử lý, **when** cả hai cùng chạy, **then** bộ đếm vé chỉ tăng **đúng một lần** (kiểm chứng BR-93).                                                                                                                       |
...
| 13. | **Lưu trữ tệp ảnh dùng dịch vụ bên thứ ba** (Cloudinary hoặc Supabase Storage, gói miễn phí) chứ không lưu trên máy chủ ứng dụng — xem FR-40, BR-105. Hệ quả đã lường trước: hệ thống phụ thuộc vào tính sẵn sàng của dịch vụ ngoài, và ảnh đã tải lên **không** bị xoá tự động khi sự kiện bị huỷ hay tài khoản bị vô hiệu hoá (dọn dẹp thủ công định kỳ, không có cơ chế garbage collection trong phạm vi đồ án). Lựa chọn này đánh đổi tính tự chủ lấy việc không phải tự xử lý lưu trữ bền vững, CDN và tối ưu ảnh — những việc không đóng góp gì vào giá trị học thuật của đồ án. |
